...
const outlinedLabelsPlugin = {
  ...
  afterDraw(chart) {
    ...
    labels.forEach((label, i) => {
      ...
      if (i === 0) y -= 5; // Power up slightly

      // Average between previous Speed (-60) and Defense (-40) = -50
...
const inputValuePlugin = {
  ...
  afterDraw(chart) {
    ...
    labels.forEach((label, i) => {
      ...
      if (i === 0) y -= 20; // Power raised
      if (chart.canvas.id === 'radarChart2') {
        // Average both numeric label offsets (previously 30)
        if (i === 1 || i === 4) y -= 30;
      } else {
        if (i === 1) y += 20;
        if (i === 4) y += 20;
